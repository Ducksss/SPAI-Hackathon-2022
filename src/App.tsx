...
class App extends Component {
	...
	runModel = async () => {
		try {
			...
			let [tensorOutput, inferenceTime] = await runModelUtils.runModel(
				this.session,
				preProcessedData
			);
			if (!!tensorOutput) {
				this.setState({
					output: getPredictedClass(
						tensorOutput.data as Float32Array
					),
					sessionRunning: false,
					inferenceTime,
				});
				...
				<Styled.ImagesRow>
					{SQUEEZENET_IMAGE_URLS.map((item) => (
						<Styled.ImageContainer
							data-selected={
								this.state.selectedImage === item.value
							}
							data-background={item.value}
							key={item.value}
							onClick={() => this.loadImageToCanvas(item.value)}
						/>
					))}
				</Styled.ImagesRow>
				<Styled.Description>
					{this.state.modelLoading &&
						"Loading Model, this make take a bit longer please hold"}{" "}
					{this.state.sessionRunning && "Processing Image"}
				</Styled.Description>
				<Styled.ResultContainer>
					{this.state.output
						? this.state.output.map((item: OutputItem) => {
								return (
									<Styled.ResultItem key={item.id}>
										<p>{item.name}</p>
										<span>
